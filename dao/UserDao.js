...
// 更新用户
function UpdateUser(id, name, phone, date, address, sex) {
  return new Promise((resolve, reject) => {
    const data = poolFn(`UPDATE user SET name='${name}', phone='${phone}', date='${date}', address='${address}' , sex='${sex}' WHERE id = ${id}`)
    if (data) {
      resolve(data)
    } else {
      reject({
...
const ModifyPassword = (password, id) => {
  return new Promise((resolve, reject) => {
    const data = poolFn(`UPDATE user set password = '${password}' where id = ${id}`)
    if (data != '') {
      resolve(data)
    } else {
      reject({
